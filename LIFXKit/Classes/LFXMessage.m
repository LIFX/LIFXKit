...
- (instancetype)init
{
	if ((self = [super init]))
	{
		...
		self.path = [LFXBinaryPath pathWithSiteID:[LFXSiteID zeroSiteID] targetID:[LFXBinaryTargetID broadcastTargetID]];
...
- (BOOL)isAResponseMessage
{
	...
	(self.messageType == LX_PROTOCOL_DEVICE_STATE_DUMMY_LOAD ||
	 ...
	 self.messageType == LX_PROTOCOL_LIGHT_STATE_TEMPERATURE);
}

- (NSString *)messageTypeAsString
{
	return [NSStringFromClass([self class]) lfx_substringFromIndexIfItExists:[NSStringFromClass([LFXMessage class]) length]];
}
